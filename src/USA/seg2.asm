; AUTO-GENERATED from original USA segment 2
; segment_size=924 bytes, flags=0x0040
...
USA_TEXT SEGMENT BYTE PUBLIC 'CODE'
...
; @ANALYSIS_v1
;----------------------------------------------------------------------
; sub_013A -- offset 0x013A -- 160 instr
...
; @ANALYSIS_v1
;----------------------------------------------------------------------
; sub_0254 -- offset 0x0254 -- 118 instr
; Iterador con instrucciones de string (movsb/scasb/...) (118 instr).
; tags: complex_iterator, far, string_op
; calls (intra): sub_0361
;----------------------------------------------------------------------
L_0254:
...
L_0263:
        ...
        xor     ax, ax                          ; 33 C0
        xchg    byte ptr [0x3a6], al            ; 86 06 A6 03
...
L_02A9:
        ...
        jb      L_02F6                          ; 72 25
        xor     bh, bh                          ; 32 FF
        mov     bh, byte ptr [bx + 0x39a]       ; 8A BF 9A 03
        ...
        cmp     al, bl                          ; 3A C3
        jne     L_02A4                          ; 75 B5
        mov     byte ptr [0x3a6], 1             ; C6 06 A6 03 01
        jmp     L_02A4                          ; EB AE
L_02F6:
        mov     byte ptr [0x330], al            ; A2 30 03
...
; @ANALYSIS_v1
;----------------------------------------------------------------------
; sub_0361 -- offset 0x0361 -- 14 instr
; Wrapper que delega en sub_0380.
; tags: wrapper
; callers: sub_0254
; calls (intra): sub_0380
;----------------------------------------------------------------------
L_0361:
        xor     bx, bx                          ; 33 DB
        mov     al, 0x40                        ; B0 40
        mov     bl, 0x14                        ; B3 14
        call    L_0380                          ; E8 16 00
        mov     al, 0x10                        ; B0 10
        mov     bl, 0x91                        ; B3 91
        call    L_0380                          ; E8 0F 00
        mov     al, 0x20                        ; B0 20
        mov     bl, 0x90                        ; B3 90
        call    L_0380                          ; E8 08 00
        mov     ah, 1                           ; B4 01
        pushf                                   ; 9C
...
; @ANALYSIS_v1
;----------------------------------------------------------------------
; sub_0380 -- offset 0x0380 -- 10 instr
